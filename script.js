...
let div = document.getElementById('convertedCard');
...
function findof() {
    ...
    let lowerCase = typing.value.toLowerCase();
    let foundData = '';
    vehicleInformation.filter((item) => {
        if (item.code.toLowerCase().includes(lowerCase)) {
            foundData += `<div class="myDiv">
        <ul> 
            <h4 class="title">VEHICLE INFORMATION </h4>
<li class="li"> City Code : ${item.code} </li>
<li class="li">Location : ${item.location} </li>
<li class="li"> Registered : ${item.type}</li>
<li class="li">District : ${item.district} </li></ul>
</div>`

        }

    });
    div.innerHTML = `${foundData}`;

}
button.addEventListener("click", findof);
findof();